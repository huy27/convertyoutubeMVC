...
namespace ConvertYoutubeToMp3.Controllers
{
    ...
    public class YouTubeToMP3Controller : ControllerBase
    {
        ...
        public YouTubeToMP3Controller(IHttpClientFactory httpClientFactory, ILoggerFactory logFactory)
        {
            _httpClientFactory = httpClientFactory;
            _logger = logFactory.CreateLogger<YouTubeToMP3Controller>(); ;
        ...
        public async Task<IActionResult> ConvertToMP4(string videoId, string quality = "auto")
        {
            try
            {
                ...
                _logger.LogWarning(httpClient.DefaultRequestHeaders.ToString());
                _logger.LogWarning(content.Headers.ToString());
                ...
                //var aaaa = await httpClient2.GetAsync(videoDetail.Dlink, HttpCompletionOption.ResponseHeadersRead);
                //var fileBytes = await aaaa.Content.ReadAsStreamAsync();
                //return File(fileBytes, "application/octet-stream", videoDetail.Title);
